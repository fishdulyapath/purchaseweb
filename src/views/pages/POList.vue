    ...
    <div class="p-4">
        ...
        <Dialog v-model:visible="showDetailDialog" modal header="รายละเอียดใบสั่งซื้อ" :style="{ width: '98%', maxWidth: '1000px' }" :draggable="false" :closable="!isSaving">
            ...
            <template #footer>
                ...
                <Button label="บันทึก" icon="pi pi-save" severity="success" @click="saveDetail" :loading="isSaving" :disabled="editItems.length === 0" />
            </template>
        </Dialog>
        <!-- Select PR Dialog -->
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody>
                        <tr v-for="pr in prDocs" :key="pr.doc_no"
                            ...
                            <td class="px-3 py-2">{{ pr.cust_code }}</td>
